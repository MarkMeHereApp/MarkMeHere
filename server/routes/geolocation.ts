...
export const geolocationRouter = router({
    ...
    .mutation(async (requestData) => {
        ...
        try{
            const professorGeolocatinCoordinates = await prisma.professorLectureGeolocation.findUnique({
                where:{
            ...
            if(!professorGeolocatinCoordinates){
                return{success: false, message: 'no coordinates found for this professor ID'}
            }

            const professorLatitude = professorGeolocatinCoordinates.lectureLatitude
            const professorLongtitude = professorGeolocatinCoordinates.lectureLongitude
            ...
            const distanceBetween2Points = (profLat: number, profLong: number, studLat: number, studLong: number) => {
              ...
              const calculateDistance = distanceBetween2Points(professorLatitude,professorLongtitude,studentLatitude,studentLongtitude)
