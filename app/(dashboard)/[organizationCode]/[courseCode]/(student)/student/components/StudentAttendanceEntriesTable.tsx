...
const StudentAttendanceEntriesTable: React.FC<
  StudentAttendanceEntriesTableProps
> = ({ attendanceEntries }) => {
  const [extendedEntries, setExtendedEntries] = React.useState<
    ExtendedAttendanceEntry[]
  >([]);
            ...
            {extendedEntries.map((entry) => (
              <TableRow key={entry.id}>
                <TableCell className="font-normal">
                  {entry.LectureDate &&
                    format(new Date(entry.LectureDate), 'LLL dd, y')}
                </TableCell>
                <TableCell className="font-normal">
                  {format(entry.dateMarked, 'LLL dd, y')}
                </TableCell>
